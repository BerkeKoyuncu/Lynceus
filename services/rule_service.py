...
def evaluate_rules_for_host(host, asset, user_id, prev_ports=None):
    """
    ...
    for rule in rules:
        ...
        if matched:
            ...
            existing_finding = SecurityFinding.query.filter_by(
                ...
                cve=None  # Since it's generated by rule rather than CVE scanner
            ).first()
            ...
            if existing_finding:
                ...
                if existing_finding.status in ["resolved", "false_positive", "accepted_risk"]:
                    existing_finding.status = "open"  # Auto-reopen!
            else:
...
def evaluate_cve_findings(host, asset, cve_list):
    """
...
def calculate_network_risk_score(user_id):
    ...
    findings = SecurityFinding.query.filter_by(status="open").all()
